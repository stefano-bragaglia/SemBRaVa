...
/**
 * The {@link PerformanceMonitor} measures increments to a counter. During the
 * computation it prints out current and average throughput per second. After
 * the computation is done it prints a final performance report.
 * <p>
 * <b>Note:</b> This class is not thread safe. <br>
...
public class PerformanceMonitor {
	...
	public void startAndPrintThroughput() {
		...
		final Runnable beeper = new Runnable() {
			...
			public void run() {
				...
				long timePassedSinceLastCount = System.nanoTime() - lastTimeStamp;

				double currentThroughput;

				if (timePassedSinceLastCount > 0) {
					currentThroughput = deltaCount / ((double) timePassedSinceLastCount / 1000000000);
				} else {
					currentThroughput = 0;
				}

				long totalTimePassed = System.nanoTime() - startTime;

				double averageThroughput;
				if (totalTimePassed > 0) {
					averageThroughput = counter / (((double) totalTimePassed) / 1000000000);
				} else {
					averageThroughput = 0;
				}

				out.printf("current: %.1f " + unit + "/s avg: %.1f " + unit + "/s total: %d " + unit + "/s%n", currentThroughput, averageThroughput, counter);
	...
	public void stopAndPrintFinalResult() {
		...
		double average;
		if (timePassed > 0) {
			average = counter / (timePassed / 1000000000d);
		} else {
			average = 0;
		}
		...
		out.println("Total: " + counter + " " + unit + "/s");
		out.printf("Runtime: %.3fs%n", timePassed / 1000000000d);
	}
